...
impl Default for TlsConfig {
    fn default() -> Self {
        Self {
            cert_path: PathBuf::from("/etc/synapsedb/tls/node.crt"),
            key_path: PathBuf::from("/etc/synapsedb/tls/node.key"),
            ca_cert_path: PathBuf::from("/etc/synapsedb/tls/ca.crt"),
...
mod tests {
    ...
    fn cert_validation_states() {
        let valid = CertValidation::Valid(PeerIdentity {
            common_name: "node-1.synapsedb.local".into(),
            san_dns: vec!["node-1.synapsedb.local".into()],
            is_node: true,
        });
        assert!(valid.is_valid());
        assert_eq!(
            valid.identity().unwrap().common_name,
            "node-1.synapsedb.local"
        );
